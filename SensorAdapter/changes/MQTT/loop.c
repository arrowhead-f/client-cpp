...
static void loop_handle_reads_writes(struct mosquitto_db *db, struct pollfd *pollfds, node *providerTopicsTree);
...
int mosquitto_main_loop(struct mosquitto_db *db, mosq_sock_t *listensock, int listensock_count, int listener_max, node *providerTopicsTree)
{
#ifdef WITH_SYS_TREE
	...
	while(run){
		...
		if(fdcount == -1){
			...
			loop_handle_reads_writes(db, pollfds, providerTopicsTree);
...
#endif
			do{
				if(_mosquitto_packet_read(db, context, providerTopicsTree)){
					do_disconnect(db, context);
					continue;
				}
