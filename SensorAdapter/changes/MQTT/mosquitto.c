...
{
/*
	FILE *pMyIPFile = fopen("./myip", "r");
	char line[256];

	if( fgets(line, sizeof(line)-1, pMyIPFile) != NULL)
	    providerTopicsTree = newNode(line);
	else
	    providerTopicsTree = newNode("unknown_IP");

	fclose(pMyIPFile);
*/

     providerTopicsTree = newNode("192.168.42.1");
	...
	run = 1;
	rc = mosquitto_main_loop(&int_db, listensock, listensock_count, listener_max, providerTopicsTree);
