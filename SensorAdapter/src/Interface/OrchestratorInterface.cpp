...
bool OrchestratorInterface::init_OrchestratorInterface( string ini_file )
{
	...
	if(ADDRESS.size() != 0){
	    ...
	    if ( MakeServer(PORT) ) {
		printf("Error: Unable to start OrchestratorInterface (%s:%d)!\n", ADDRESS.c_str(), PORT);
		return false;
	    }

	    printf("OrchestratorInterface started - %s:%d\n", ADDRESS.c_str(), PORT);
	}
	else{
...
bool OrchestratorInterface::init_OrchestratorInterface( string _sHttpUri, string _sHttpsUri, string _sAddr, string _sAddr6, uint16_t _uPort )
{
	OR_BASE_URI       = _sHttpUri;
	OR_BASE_URI_HTTPS = _sHttpsUri;
	ADDRESS           = _sAddr;
	ADDRESS6          = _sAddr6;
	PORT              = _uPort;
